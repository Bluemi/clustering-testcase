"""
The implementation of this algorithm is used from https://github.com/spencerkent/generalized-lloyd-quantization/tree/6d27b1b1a16a128104f224b06ee8361f6ed600d9
"""
import numpy as np
...
class GeneralizedLloyd(Algorithm):
    ...
    def cluster(self, points: np.ndarray):
        ...
        init_assignment_points = np.random.normal(size=(self.num_chunks, num_dim)) + 127
        init_assignment_codeword_lengths = np.random.normal(size=self.num_chunks)
        centers_whitened, cluster_assignments, _, _, _ = compute_quantization(
            whitened_points, init_assignment_points, init_assignment_codeword_lengths
        )
        centers = centers_whitened * np.std(points, axis=0)
        clustered_points = centers[cluster_assignments]

        return centers, clustered_points

    def name(self) -> str:
        return 'Generalized Lloyd'


# Taken part starts here
"""
...
def partition_with_drops(raw_vals, a_vals, c_lengths, l_weight):
  ...
  cword_probs = calculate_assignment_probabilities(c_assignments,
                                                   a_vals.shape[0])
...
def calculate_assignment_probabilities(assignments, num_clusters):
  """
